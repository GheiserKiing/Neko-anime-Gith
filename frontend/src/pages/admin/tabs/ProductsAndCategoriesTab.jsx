...
export default function ProductsAndCategoriesTab() {
  ...
  // — Formularios, flags y datos temporales —
  ...
  // — Toggle formularios de producto y categoría —
  const toggleProdForm = (prod) => {
    if (prod) {
      const existing = (prod.images || []).map((url) => {
  ...
  // — Selección y borrado de imágenes en el formulario —
  ...
  // — Guardar producto y procesar imágenes —
  const saveProduct = async () => {
    ...
    if (formProd.newImages.length || formProd.removedImages.length) {
      ...
      try {
        const url = `/api/products/${saved.id}/images`;
        console.log(`▷ POST ${url}`, {
          removed: formProd.removedImages,
          cover_index: formProd.coverIndex,
          newFiles: formProd.newImages.map((f) => f.name),
        });
        const result = await uploadProductImages(saved.id, fd);
        console.log("✔️ Imágenes procesadas:", result);
        saved.cover_image_url = result.cover_image_url;
      } catch (err) {
        console.error("❌ Error procesando imágenes:", err);
        alert("Error al procesar imágenes. Mira la consola.");
      }
    }

    // 3) Cerrar form y recargar lista
    setOpenProdForm(false);
    const prods = await fetchProducts();
    setProducts(prods);
  };

  // — Otras acciones: eliminar producto, CRUD categorías/subcategorías —
  ...
  const editCategory = (c) => { setEditingCat(c); setCatName(c.name); };
  ...
  const saveSub = async () => {
    ...
    if (editingSub) await updateSubcategory(editingSub.catId, editingSub.sub.id, { name: subName.trim() });
  ...
  // — Popover info —
  const showInfo = (e, p) => { setInfoAnchor(e.currentTarget); setInfoProd(p); };
  const hideInfo = () => { setInfoAnchor(null); setInfoProd(null); };
  ...
  return (
    <Box p={2}>
      ...
      <Box mb={2} display="flex" justifyContent="space-between" alignItems="center">
        <Box>
          <Button variant="contained" startIcon={<AddIcon />} onClick={() => toggleProdForm(null)}>
            Nuevo Producto
          </Button>
          <Button variant="outlined" sx={{ ml: 1 }} startIcon={<AddIcon />} onClick={toggleCatForm}>
            Categorías
          </Button>
        </Box>
      ...
      <Collapse in={openCatForm}>
        <Box mb={2} p={2} bgcolor="#f0f4f8">
          ...
          <Typography variant="subtitle2" gutterBottom>Categorías existentes</Typography>
          {categories.map((c) => (
            <Box key={c.id} display="flex" alignItems="center" mb={1}>
              <Box flexGrow={1}>{c.name}</Box>
              <IconButton size="small" onClick={() => editCategory(c)}><EditIcon fontSize="small"/></IconButton>
              <IconButton size="small" onClick={() => removeCategory(c.id)}><DeleteIcon fontSize="small"/></IconButton>
            </Box>
          ))}
          <Typography variant="subtitle2" gutterBottom mt={2}>Subcategorías existentes</Typography>
          {Object.entries(subcatsMap)
            .flatMap(([cid, subs]) => subs.map((s) => ({ cid, s })))
            .map(({ cid, s }) => (
              <Box key={s.id} display="flex" alignItems="center" mb={1}>
                <Box flexGrow={1}>
                  <strong>{categories.find((c) => c.id === +cid)?.name}</strong>: {s.name}
                </Box>
                <IconButton size="small" onClick={() => editSubcategory(categories.find((c) => c.id === +cid), s)}>
                  <EditIcon fontSize="small"/>
                </IconButton>
                <IconButton size="small" onClick={() => removeSubcategory(+cid, s.id)}>
        ...
        <Box mb={2} p={2} bgcolor="#fafafa">
          ...
          <Box display="flex" flexWrap="wrap" gap={2} mb={2}>
            <TextField
              label="SKU"
              value={formProd.sku}
              onChange={(e) => setFormProd((fp) => ({ ...fp, sku: e.target.value }))}
            />
            <TextField
              label="Nombre"
              value={formProd.name}
              onChange={(e) => setFormProd((fp) => ({ ...fp, name: e.target.value }))}
            />
            <TextField
              label="Descripción"
              multiline rows={3} fullWidth
              value={formProd.description}
              onChange={(e) => setFormProd((fp) => ({ ...fp, description: e.target.value }))}
            />
            <TextField
              label="Precio €" type="number"
              value={formProd.price}
              onChange={(e) => setFormProd((fp) => ({ ...fp, price: e.target.value }))}
            />
            <TextField
              label="Stock" type="number"
              value={formProd.stock}
              onChange={(e) => setFormProd((fp) => ({ ...fp, stock: e.target.value }))}
            />
            <FormControl>
              <InputLabel>Categoría</InputLabel>
              <Select
            ...
            <FormControl>
              ...
              <Select
                ...
                onChange={(e) =>
                  setFormProd((fp) => ({ ...fp, subcategory: e.target.value }))
                }
              >
              ...
              <Box key={idx} position="relative">
                <Avatar
                  ...
                  sx={{
                    ...
                    border: idx===formProd.coverIndex?"4px solid #1976d2":"2px solid #ccc",
                    opacity: idx===formProd.coverIndex?1:0.7,
                    cursor:"pointer"
                  }}
                  onClick={()=>setFormProd(fp=>({...fp,coverIndex:idx}))}
                />
                <IconButton
                  size="small"
                  sx={{position:"absolute",top:-10,right:-10,background:"#fff"}}
                  onClick={()=>setFormProd(fp=>({
                    ...fp,
                  ...
                  sx={{
                    ...
                    border:
                      idx+formProd.existingImages.length===formProd.coverIndex
                        ?"4px solid #1976d2":"2px solid #ccc",
                    opacity:
                      idx+formProd.existingImages.length===formProd.coverIndex
                        ?1:0.7,
                    cursor:"pointer"
                  }}
                  onClick={()=>setFormProd(fp=>({...fp,coverIndex:idx+fp.existingImages.length}))}
                />
                <IconButton
                  size="small"
                  sx={{position:"absolute",top:-10,right:-10,background:"#fff"}}
          ...
          <Button variant="contained" onClick={saveProduct}>Guardar Producto</Button>
        </Box>
      </Collapse>
        ...
        <Table>
          <TableHead>
            <TableRow sx={{ bgcolor:"#333" }}>
              {["ID","Imagen","SKU","Nombre","Precio","Stock","Cat.","Sub.","Info","Acciones"].map((h)=>(
          ...
          <TableBody>
            {displayed.map((p,idx)=>(
              <TableRow key={p.id} sx={{bgcolor:idx%2?"#fafafa":"#fff","&:hover":{bgcolor:"#f0f0f0"}}}>
                <TableCell>{p.id}</TableCell>
                <TableCell><Avatar src={p.cover_image_url} variant="square" sx={{width:56,height:56}}/></TableCell>
                <TableCell>{p.sku}</TableCell>
                <TableCell>{p.name}</TableCell>
                <TableCell>€{p.price.toFixed(2)}</TableCell>
                <TableCell>{p.stock}</TableCell>
                <TableCell sx={{bgcolor:"#fffbeb"}}>{categories.find(c=>c.id===p.category_id)?.name}</TableCell>
                <TableCell sx={{bgcolor:"#eefbff"}}>{subcatsMap[p.category_id]?.find(s=>s.id===p.subcategory_id)?.name}</TableCell>
                <TableCell>
          ...
          {infoProd && (
            <>
              <Typography variant="body2">
                <strong>Creado:</strong> {new Date(infoProd.created_at).toLocaleDateString()}
              </Typography>
              <Typography variant="body2"><strong>SKU:</strong> {infoProd.sku}</Typography>
              <Typography variant="body2">
                <strong>Categoría:</strong> {categories.find(c=>c.id===infoProd.category_id)?.name}
              </Typography>
              {infoProd.subcategory_id && (
                <Typography variant="body2">
                  <strong>Subcat.:</strong> {subcatsMap[infoProd.category_id]?.find(s=>s.id===infoProd.subcategory_id)?.name}
                </Typography>
              )}
