...
export default function ProductDialog({ open, product, onClose, onSave }) {
  ...
  const [coverIndex, setCoverIndex]   = useState(0);      // este cambio estado para índice de portada
  ...
  useEffect(() => {
    if (product) {
      ...
      setFiles([]);
      setCoverIndex(0);                                   // este cambio reinicia coverIndex en edición
    } else {
      setName(""); setDescription(""); setPrice(""); setStock("");
      setCategoryId(""); setSubcategoryId(""); setFiles([]);
      setCoverIndex(0);                                   // este cambio reinicia coverIndex en nuevo
    }
  }, [product]);
  ...
  const handleFiles = (e) => {
    ...
    setFiles(arr);
    setCoverIndex(0);                                     // este cambio selecciona la primera por defecto
  };
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      // no enviamos cover_index aquí, lo enviaremos en uploadProductImages
    };
    try {
      ...
      if (files.length > 0) {
        await uploadProductImages(saved.id, files, coverIndex); // este cambio pasa coverIndex
      }
      onSave();
    } catch (err) {
